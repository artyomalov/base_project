...
class AuthUserServices:
    ...
    async def issue_new_access_if_refresh_is_valid(self, token: bytes) -> bytes:
        ...
        user_dto = await self._get_user_and_check_if_user_is_active(user_id=user_id)
...
class UserServices:
    ...
    async def get_user(self, user_id) -> UserSchema:
        user = await self.db_requests.get_user(user_id=user_id)
        return user
    ...
    async def delete_user(
        ...
        await self.db_requests.delete_user(user_id=user_id)
